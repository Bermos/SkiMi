...
public class CharacterInputController : MonoBehaviour
{
    ...
    protected bool TutorialMoveCheck(int tutorialLevel)
    {
        ...
        return (
        !TrackManager.instance.isTutorial ||
    ...
    protected void Update()
    {
        ...
        if (m_Sliding)
        {
            ...
            float correctSlideLength =
                slideLength * (1.0f + trackManager.speedRatio);
            float ratio =
                (trackManager.worldDistance - m_SlideStart) /
                correctSlideLength;
            if (ratio >= 1.0f)
            {
        ...
        if (m_Jumping)
        {
            if (trackManager.isMoving)
            {
                ...
                float correctJumpLength =
                    jumpLength * (1.0f + trackManager.speedRatio);
                float ratio =
                    (trackManager.worldDistance - m_JumpStart) /
                    correctJumpLength;
                if (ratio >= 1.0f)
                {
                    m_Jumping = false;
                ...
                else
                {
                    verticalTargetPosition.y =
                        Mathf.Sin(ratio * Mathf.PI) * jumpHeight;
                }
            }
            else if (
                !AudioListener.pause //use AudioListener.pause as it is an easily accessible singleton & it is set when the app is in pause too
            )
            {
                verticalTargetPosition.y =
                    Mathf
                        .MoveTowards(verticalTargetPosition.y,
                        0,
        ...
        characterCollider.transform.localPosition =
            Vector3
                .MoveTowards(characterCollider.transform.localPosition,
        ...
        if (
            Physics
                .Raycast(characterCollider.transform.position + Vector3.up,
                Vector3.down,
                out hit,
                k_ShadowRaycastDistance,
                m_ObstacleLayer)
        )
        {
            blobShadow.transform.position =
                hit.point + Vector3.up * k_ShadowGroundOffset;
        }
        else
        {
    ...
    public void Jump()
    {
        ...
        if (!m_Jumping)
        {
            ...
            float animSpeed =
                k_TrackSpeedToJumpAnimSpeedRatio *
    ...
    public void ChangeLane(int direction)
    {
        ...
        m_TargetPosition =
            new Vector3((m_CurrentLane - 1) * trackManager.laneOffset, 0, 0);
    }
